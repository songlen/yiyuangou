    ...
    public function prepareOrder(){
        ...
        foreach ($goodsInfo as $item) {
            ...
            if($actInfo['limit_num'] > 0){
                ...
                $can_buy_num = $actInfo['limit_num'] - $buyed_num - $item['num'];
                if($can_buy_num < 1) response_error('', '超出管理员限制的购买数量:'.$actInfo['limit_num'].'-'.$buyed_num.'-'.$item['num']);
            }
    ...
    public function payCallback($order_sn_gather = ''){
        ...
        if(!empty($order_sns)){
            foreach ($order_sns as $order_sn) {
                // 获取订单信息，判断是否已支付
                $order = M('order')->where('order_sn', $order_sn)->field('user_id, order_id, order_sn, pay_status, goods_price, prom_id, num, integral')->find();
